...
// The IMDb second opinion — the Go pipeline's weekly slice, keyed
// normTitle|year. Optional: cards degrade to TMDB-only when it's missing.
let imdbSlice = null;
export async function loadImdbSlice(base = '') {
  try {
    imdbSlice = await fetch(`${base}data/imdb-slice.json`).then((r) => (r.ok ? r.json() : null));
  } catch { imdbSlice = null; }
  return !!imdbSlice;
}
export function setImdbSlice(s) { imdbSlice = s; } // Node builds inject it directly
const imdbOf = (title, year) => {
  const hit = imdbSlice?.[`${normTitle(title)}|${year}`];
  return hit ? { rating: hit[0], votes: hit[1] } : null;
};

const asCard = (c, why) => {
  const year = (c.release_date || '').slice(0, 4) || c.year || '';
  return {
    tmdbId: c.id, title: c.title, year,
    poster: c.poster_path || null,
    tmdb: c.vote_average ? { rating: Math.round(c.vote_average * 10) / 10 } : null,
    imdb: imdbOf(c.title, year),
    runtime: c.runtime || 0,
...
export async function buildShelves(data, films, syllabus, key, onProgress = () => {}) {
  ...
  // the floor: IMDb's word when we have it (≥6.8), else TMDB's (≥6.8)
  const quality = detailed.filter((c) => {
    const imdb = imdbOf(c.title, (c.release_date || '').slice(0, 4));
    return imdb ? imdb.rating >= 6.8 || (c.vote_average || 0) >= 7.2 : (c.vote_average || 0) >= 6.8;
  });
