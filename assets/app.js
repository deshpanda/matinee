// Matinée — your film life, developed in your browser.
// No accounts, no server, no uploads that leave this machine: the visitor
// drops their Letterboxd export, TMDB garnish is fetched from THEIR browser,
// and everything computed is stored in THEIR IndexedDB. We never see it.
//
// Five pages render slices of the same computed object: overview (this
// file's home + the landing/darkroom), stats/, next/, school/, archive/.
...
import { parseExport, uniqueFilms } from '../lib/export-parse.js';
...
// Director pages use diacritic-stripped kebab slugs (kieślowski→kieslowski, wong-kar-wai).
...
async function main() {
  initTip();
  const v = await kv.get('vault').catch(() => null);
  if (v) return renderPage(v);
  if (PAGE !== 'overview') { location.href = BASE || './'; return; }
  renderLanding();
}

// ---------- the landing / the darkroom ----------
function renderLanding(message = '') {
  root.innerHTML = '';
  const gate = h('div', 'gate');
  const card = h('div', 'gate-card gate-wide');
  card.appendChild(h('div', 'reel', '\u25cf \u25cf \u25cf'));
  card.appendChild(h('h1', null, BRAND));
  card.appendChild(h('p', 'tag', 'your film life, developed in your browser'));

  const drop = h('div', 'dropzone');
  drop.appendChild(h('p', 'drop-big', 'Drop your Letterboxd export here'));
  drop.appendChild(h('p', 'drop-small', 'the ZIP from letterboxd.com \u2192 Settings \u2192 Data \u2192 Export, or the CSVs inside it'));
  const input = h('input');
  input.type = 'file';
  input.multiple = true;
  ...
  drop.addEventListener('drop', (e) => {
    ...
    develop([...e.dataTransfer.files]);
  });
  input.addEventListener('change', () => develop([...input.files]));
  ...
  demoBtn.addEventListener('click', async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(BASE + 'data/demo.json', { cache: 'no-store' });
      const demo = await res.json();
      demo.isDemo = true;
      await kv.put('vault', demo);
      renderPage(demo);
    } catch { err.textContent = 'The demo print is missing from this deployment.'; }
  });
...
async function develop(files) {
  ...
  try {
    ...
    const data = parseExport(texts);
    stage(`${data.diary.length} diary entries, ${data.watched.length} films. Fetching film data\u2026`);
    const films = await enrichFilms(uniqueFilms(data), TMDB_KEY, {
      cache: tmdbCache,
      onProgress: (d, t, title) => stage(`Developing ${d}/${t} \u2014 ${title}`, d / t * 0.7),
    });
    ...
    renderLanding(e.message || 'That did not develop. Is it a Letterboxd export?');
  }
}
...
function renderDarkroom() {
  ...
  card.appendChild(h('div', 'reel', '\u25cf \u25cf \u25cf'));
  card.appendChild(h('h1', null, 'Developing'));
  const line = h('p', 'tag', 'reading the export\u2026');
  card.appendChild(line);
  const barWrap = h('div', 'dev-bar');
  const bar = h('div', 'dev-bar-fill');
  barWrap.appendChild(bar);
  card.appendChild(barWrap);
  card.appendChild(h('p', 'gate-note', 'All of this is happening inside your browser. Leave the tab open.'));
  gate.appendChild(card);
  root.appendChild(gate);
  return (text, frac) => {
...
// ZIP (via vendored fflate) or loose CSVs \u2192 {filename: text}
  ...
  for (const f of files) {
    if (/\.zip$/i.test(f.name)) {
      ...
      for (const [path, bytes] of Object.entries(entries)) {
        ...
        if (/\.csv$/i.test(base) && !texts[base]) texts[base] = dec.decode(bytes);
      }
    } else if (/\.csv$/i.test(f.name)) {
      texts[f.name] = await f.text();
    }
  }
  return texts;
}
...
function masthead(v) {
  ...
  title.innerHTML = 'Mati<span>née</span>';
  ...
  right.appendChild(h('span', 'meta',
    (v.isDemo ? 'demo print' : v.generatedAt ? `developed ${v.generatedAt}` : 'your print')));
  const lock = h('button', null, 'Eject');
  lock.title = 'Forget this print — wipes everything stored in this browser';
  lock.addEventListener('click', async () => { await wipeAll(); location.href = BASE || './'; });
